...
const (
	...
	apiPath      = "/service/audioPlayer/api/v1/secure/playlist/get/all"
...
var (
	requestBody = `{"page":{"pageIndex":1,"pageSize":20}}` // 分页参数
)
...
func main() {
	...
	for res := range attacker.Attack(targeter, rate, testDuration, "全歌单获取压测") {
		metrics.Add(res)
